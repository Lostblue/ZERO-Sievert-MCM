...
if instance_exists(obj_player)
{
    ...
    if (obj_player.state == gml_Script_scr_player_state_teleport)
    {
        _bw = 220
        _bh = 120
        var _bx = (240 - (_bw / 2))
        var _by = (135 - (_bh / 2))
        scr_draw_box(s_box_skill1, _bx, _by, _bw, _bh, 1)
        draw_set_font(font_quest)
        draw_set_halign(fa_center)
        draw_set_valign(fa_top)
        draw_text(240, (global.tp_to_npc_starty - 20), "Teleport to:")
        draw_set_font(font_quest)
        draw_set_halign(fa_left)
        draw_set_valign(fa_top)
        for (i = 0; i < array_length(global.tp_to_npc_x); i++)
        {
            var _tx = global.tp_to_npc_startx
            var _ty = global.tp_to_npc_starty
            _h = 12
            _c = c_white
            if (i == global.tp_to_npc_hover)
                _c = 0x91F2FF
            var _draw = 1
            var _text = ((("[" + string((i + 1))) + "] - ") + global.tp_to_npc_name[i])
            if (global.tp_to_npc_can_tp[i] == 0)
                _draw = 0
            if (_draw == 1)
                draw_text_color(_tx, (_ty + (_h * i)), _text, _c, _c, _c, _c, 1)
        }
    }
